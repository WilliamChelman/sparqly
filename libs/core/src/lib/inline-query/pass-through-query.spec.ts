...
import { parseSourceSpecs, type ParsedEndpointSource } from '../sources';
import {
  startFakeSparqlEndpoint,
  type FakeSparqlEndpoint,
} from '../test/fake-sparql-endpoint';
import { runPassThroughQueryResult } from './pass-through-query';
...
function endpointSource(url: string): ParsedEndpointSource {
  return parseSourceSpecs([{ id: 'live', endpoint: url }])[0] as ParsedEndpointSource;
}
...
describe('runPassThroughQueryResult — endpoint pass-through', () => {
  ...
  it('forwards the inline query verbatim to the endpoint and returns the result as a Store', async () => {
    ...
    const result = await runAgainstEndpoint(endpointSource(endpoint.url), QUERY);
  ...
  it('forwards bearer auth from the endpoint source as `Authorization: Bearer <token>`', async () => {
    ...
    const source = parseSourceSpecs([
      { id: 'live', endpoint: endpoint.url, auth: { type: 'bearer', token: 'tk-1' } },
    ])[0] as ParsedEndpointSource;

    await runAgainstEndpoint(source, 'SELECT ?s ?p ?o WHERE { ?s ?p ?o }');
  ...
  it('forwards custom headers from the endpoint source verbatim', async () => {
    ...
    const source = parseSourceSpecs([
      { id: 'live', endpoint: endpoint.url, headers: { 'X-Tenant': 'acme' } },
    ])[0] as ParsedEndpointSource;
  ...
  it('honours `timeoutMs` on the endpoint source (slow endpoint surfaces an endpoint-fetch error)', async () => {
    endpoint = await startFakeSparqlEndpoint(async () => {
      await new Promise((resolve) => setTimeout(resolve, 250));
      return { body: SPARQL_JSON_TWO_BINDINGS };
    });
    const source = parseSourceSpecs([
      { id: 'live', endpoint: endpoint.url, timeoutMs: 25 },
    ])[0] as ParsedEndpointSource;
  ...
  it('returns an endpoint-fetch variant naming the endpoint when the remote 500s', async () => {
    endpoint = await startFakeSparqlEndpoint(() => ({ status: 500, body: 'boom' }));
