...
/**
 * Per-variant exit-code map for `hash`. Stable across releases — scripts grep
 ...
 *   2   compare-mode error (set by hashSpec.exitCode when compareWith is set)
 ...
 */
export function hashErrorExitCode(error: SourceError | TargetError): number {
  switch (error.kind) {
 ...
 * colour. ADR-0024.
 */
export function decorateHashError(
  error: SourceError | TargetError,
  { color }: DecorateOptions,
): string {
  const body = formatHashError(error);
  return color ? `${ANSI_RED}${body}${ANSI_RESET}` : body;
}

function formatHashError(error: SourceError | TargetError): string {
...
/**
 * Silent wrapper carried out of the hash handler so the runner skips its
 * default `error: <message>` line — the CLI itself writes the ANSI-decorated
 * `formatHashError` body and uses `hashSpec.exitCode` to route the variant
 * through `hashErrorExitCode`.
 */
export class HashErrorSignal extends Error {
  readonly silent = true;
  constructor(public readonly hashError: SourceError | TargetError) {
    super(formatHashError(hashError));
    this.name = 'HashErrorSignal';
  }
}
