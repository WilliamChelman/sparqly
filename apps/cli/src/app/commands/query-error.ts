...
/**
 * Per-variant exit-code map for `query`. Stable across releases — scripts grep
 ...
 */
export function queryErrorExitCode(error: SourceError | TargetError): number {
  switch (error.kind) {
 ...
 * colour. ADR-0024.
 */
export function decorateQueryError(
  error: SourceError | TargetError,
  { color }: DecorateOptions,
): string {
  const body = formatQueryError(error);
  return color ? `${ANSI_RED}${body}${ANSI_RESET}` : body;
}

function formatQueryError(error: SourceError | TargetError): string {
...
/**
 * Silent wrapper carried out of the query handler so the runner skips its
 * default `error: <message>` line — the CLI itself writes the ANSI-decorated
 * `formatQueryError` body and uses `querySpec.exitCode` to route the variant
 * through `queryErrorExitCode`.
 */
export class QueryErrorSignal extends Error {
  readonly silent = true;
  constructor(public readonly queryError: SourceError | TargetError) {
    super(formatQueryError(queryError));
    this.name = 'QueryErrorSignal';
  }
}
