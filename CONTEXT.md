...
- The **Describe page** issues `POST /api/describe` against the registry, which dispatches per source kind: `glob`/`view` targets materialize via `resolveSource` and run the full **describe** fixpoint against the resulting Store (`describeStore`); `endpoint` targets run a depth-0 fetch over the wire (`describeEndpoint`) — direct neighbours only, deeper only via **describe expansion paths** in the request; `empty` is rejected as a top-level describe target; `reference` is rejected per ADR-0005. Per-source caps come from the **Describe block** (soft default, hard ceiling).
