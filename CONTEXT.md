...
**Describe page**:
The webapp surface at `/describe?iri=<encoded>&sources=<csv>` that runs **describe** of a seed IRI across one or more sources in the **served registry** and renders the merged result. When the served registry has a single source the picker collapses to it, mirroring the query/diff pages. Initial state on first load selects all sources; zero sources selected produces an empty result with a "Select all" affordance — `[0..n]` is honest, 0 is not aliased to "all". Backed by `POST /api/describe`.
