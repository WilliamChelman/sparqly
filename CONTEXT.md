...
**Glob index**:
The persistent on-disk index a **Disk-backed glob** materializes into — an embedded LevelDB-backed quad store under a sparqly cache directory (`<configDir>/.sparqly/index/<source-id>/` by default; the cache path is config-overridable). Carries a manifest — the matched files' paths, sizes and mtimes, the sparqly version, and the applied transform pipeline (each transform's key and config) — compared on open: a mismatch — including a re-pointed transform config — is a staleness `warn`, never a silent rebuild. Queried by the standard query engine as an ordinary RDF/JS source, so a disk-backed glob answers SPARQL identically to an in-memory one; only the memory ceiling differs.
