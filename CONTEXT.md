...
**Disk-backed glob**:
A **Glob source** declared `storage: disk`. Opt-in. Its **Materialized resolution** loads the matched files into a persistent on-disk **Glob index** instead of an in-memory `Store`, so a glob whose triples exceed RAM is queryable without an out-of-memory crash. The index is built once — in the background on first touch (see **Lazy materialization**), or ahead of time via the `sparqly index` command — and reused across process restarts while it stays fresh. A declared `graphName` transform is baked into the index at build time; `annotateSource` is a parse error on a disk-backed glob — the **Glob index** cannot persist its RDF-star quoted triples (ADR-0041). The `storage` field is valid only on glob/file sources — a parse error on endpoint, view, and empty sources — and is inherited by a **split glob**'s **File source** children, each indexed independently. A disk-backed glob carries no **Source record sidecar**, and `hash` / `diff` reject it (RDFC-1.0 canonicalization needs every quad in memory). When un-flagged, a glob whose matched bytes exceed a soft hint draws a `warn`-level boundary log pointing at `storage: disk`.
_Avoid_: "indexed glob", "big glob", "external source", conflating with **Endpoint source**

**Glob index**:
The persistent on-disk index a **Disk-backed glob** materializes into — an embedded LevelDB-backed quad store under a sparqly cache directory (`<configDir>/.sparqly/index/<source-id>/` by default; the cache path is config-overridable). Carries a manifest — the matched files' paths, sizes and mtimes, the sparqly version, the applied transform pipeline (each transform's key and config), and the index's quad count — compared on open: a mismatch — including a re-pointed transform config — is surfaced both as a boundary `warn` log and as a `stale` value of the index's **Source load state** on `serve`'s **Sources page**, never a silent rebuild. The user clears it by issuing an explicit **(Re)build index** trigger from the page. Queried by the standard query engine as an ordinary RDF/JS source, so a disk-backed glob answers SPARQL identically to an in-memory one; only the memory ceiling differs.
...
**Union default graph** (`unionDefaultGraph`):
A boolean field on a **Glob source** controlling whether SPARQL executed against that source's materialized Store treats the default graph as the union of the default graph and every named graph. Defaults to `true`. Implemented as Comunica's `unionDefaultGraph` query-context option — non-destructive: the Store is byte-identical, named graphs stay addressable via `GRAPH ?g`, and `hash`/`diff` canonicalization is unaffected. Not a **Graph-name transformation** mode — transforms rewrite quads, and a store-mutating union would double-count triples under canonicalization. Honored wherever sparqly runs SPARQL against the glob's Store: direct `query`/`serve`, and a **View**'s own query when its `from:` chain resolves down to the glob. Never propagated to a view's output Store — querying a view is querying a computed dataset, which runs with standard SPARQL semantics. Valid only on glob sources; inherited by a **split glob**'s **File source** children; a parse error on endpoint, view, and empty sources.
...
**Source load state**:
The state of a **served registry** entry as observed and exposed by `serve` (`/api/sources`, **Sources page**). One state machine per materialization mode. In-memory materialized entries (`glob`, `file`, `view`, `empty`): `not-loaded` → `loading` → `loaded` (with `quads`) or `failed`. **Disk-backed glob** entries: `not-built` → `indexing` → `ready` (with `quads` from the **Glob index** manifest) or `failed`, plus `stale` for a `ready`-shape index whose manifest mismatches files / sparqly version / transform pipeline. `endpoint` entries have no state machine — they pass through unconditionally. The labels are mode-specific rather than unified because the transition costs differ by orders of magnitude (`loading` an in-memory glob is seconds; `indexing` a disk-backed one can be minutes).
_Avoid_: "source status", a unified "idle/busy/ready" taxonomy

**Sources page**:
The webapp surface that lists every entry of the **served registry** with its current **Source load state** and, where available, materialization metrics (e.g. quad count). URL is `/sources`. Opening the page never triggers loads — **Lazy materialization** is preserved: in-memory materialized entries report `not-loaded` until they have been queried (or until the page's per-entry **Load now** affordance is used) and additionally expose **Reload** and **Unload** affordances once loaded; **Disk-backed glob** entries surface the **Glob index** state machine and expose an explicit **(Re)build index** affordance distinct from the in-memory verbs because rebuilding tears down the existing index; **Endpoint source** entries expose a click-driven **Test connection** affordance that runs an `ASK { ?s ?p ?o }` against the endpoint and reports reachability + latency for that one click only (the result is never memoized). Read-only against the registry — the page never declares new sources; the only mutations it issues are per-entry load / unload / build triggers, each gated by **Source admin actions capability** and returning `202 Accepted` (no synchronous blocking on long-running builds). Live state updates flow over a dedicated Server-Sent Events stream so the page reflects state transitions without polling. **Split glob** entries render as a single disclosable row whose synthesized **File source** children appear indented when expanded; an action on the meta cascades to its loaded children client-side, one HTTP request per child, so per-child failures surface individually. Disk-backed `indexing` rows expose a **Cancel** affordance that SIGTERM's the build child (the side-directory build pattern means cancelling a rebuild leaves the old **Glob index** intact). The destructive **(Re)build index** trigger requires a confirm dialog when issued against a `ready` or `stale` index; all other actions skip confirmation. Refinements to this definition are pending follow-up decisions (metric shape beyond quad count, SSE initial-state handshake).
...
**Source admin actions capability**:
The `sources.allowAdminActions: boolean` field on the `/api/config` envelope, declaring whether the running `serve` will accept the **Sources page**'s per-entry triggers (**Load now**, **Reload**, **Unload**, **(Re)build index**, **Test connection**). Defaulted from the same `serve --read-only` switch as **Webapp writability capability**, but kept as an independently-named flag because the two mutation classes are genuinely different — saved-query writes touch project files, source admin actions consume server resources (CPU/RAM/disk/network) — and may need to be gated independently later. When `false`, the **Sources page** hides every action affordance and the corresponding API routes reject with `403`; the underlying auto-trigger semantics (e.g. a disk-backed glob's first-query-touch background build) are unchanged.
_Avoid_: "sources writable", conflating with **Webapp writability capability**
...
- A **Glob source** carries a **Union default graph** setting (`unionDefaultGraph`, default `true`) applied as a query-context option at every SPARQL execution against its materialized Store — including a **View**'s own query when `from:` resolves to that glob — but never propagated to a view's output Store. It is orthogonal to the **Graph-name transformation**: `flatten` destructively drops graph names, whereas `unionDefaultGraph` is a non-destructive query-time option that leaves named graphs addressable.
