...
- **File config schema is derived.** Top-level "shared" fields are the union of fields used by any registered spec; per-command blocks contain only that command's fields. Unknown keys become validation errors (no more `passthrough`).
