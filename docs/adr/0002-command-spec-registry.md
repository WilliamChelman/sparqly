...
- **Universal flags are explicit composition.** `--verbose / --quiet / -o` come from a shared field bundle each spec spreads; `--config` is runner-owned and never appears in any spec's schema.
