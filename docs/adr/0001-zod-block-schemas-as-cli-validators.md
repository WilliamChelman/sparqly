...
- **Hand-rolled validators in `core` (`isGraphStrategy`, `isSparqlFormat`) called from each command.** Rejected — the enum lists were duplicated three times (`core` const, Zod enum in `schema.ts`, hand-rolled `is*` predicate) and drift was unchecked. Also forced four near-identical "unknown --flag" blocks across the command files.
- **Move all enum runtime values into `schema.ts`.** Rejected — would invert the dependency direction (`core` would depend on `apps/cli`). Domain enums whose runtime list is meaningful to non-CLI consumers (`GRAPH_STRATEGIES`, `SUPPORTED_FORMATS`) belong in `core`.

## Consequences

- `core` exposes domain-enum runtime lists as `as const` tuples (`GRAPH_STRATEGIES`, `SUPPORTED_FORMATS`); types are derived via `(typeof TUPLE)[number]`.
...
- Hand-rolled predicate validators (`isGraphStrategy`, `isSparqlFormat`, `isDiffFormat`) are removed.
