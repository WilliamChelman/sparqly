...
- **Refusals:** non-deterministic queries (`NOW`/`RAND`/`UUID`/`STRUUID`, detected by a conservative scan for an actual function call after blanking string literals, IRIs, and comments) bypass the cache — and this guard overrides `--refresh`/per-request refresh, since storing such an answer would let a later read serve it stale; errors are never cached; empty results are cached; results over `maxEntryBytes` bypass.
- **Controls:** `--no-cache` and `--refresh`; `sparqly cache clear` / `cache stats`; a per-request refresh and an `X-Sparqly-Cache: hit|miss|bypass` header under `serve`. The per-request refresh is the standard `Cache-Control: no-cache` request directive (#418) — chosen over a query param so a shareable URL can never bake in a permanent recompute — and `cache clear` over HTTP is `POST /api/cache/clear`. Caching operates normally under `serve --read-only`; only `cache clear` is gated (403 `admin-actions-disabled`).
